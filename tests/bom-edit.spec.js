...
test.describe('BOM编辑功能测试', () => {
  test('Pattern maker can edit size specs', async ({ page }) => {
    ...
    await page.goto('http://localhost:5173/boms/PENDING_CRAFT_BOM');
  ...
  test('Craft designer can edit material list', async ({ page }) => {
    ...
    await page.goto('http://localhost:5173/boms/PENDING_PATTERN_BOM');
    await page.waitForLoadState('networkidle');
  ...
  test('BOM status controls edit permissions', async ({ page }) => {
    ...
    // 访问已确认状态的BOM（应该不可编辑）
    await page.goto('http://localhost:5173/boms/CONFIRMED_BOM');
    await page.waitForLoadState('networkidle');
  ...
  test('Edit mode toggle functionality', async ({ page }) => {
    ...
    await page.goto('http://localhost:5173/boms/PENDING_CRAFT_BOM?test=toggle');
    await page.waitForLoadState('networkidle');
  ...
  test('Design assistant has access to BOM pages', async ({ page }) => {
    // 测试"设计助理"可以访问BOM页面
    await loginAs(page, 'design_assistant');
    
    await page.goto('http://localhost:5173/boms/TEST001');
    await page.waitForLoadState('networkidle');

    // 验证页面加载成功，显示BOM信息
    await expect(page.locator('.page-title')).toContainText(/TEST001/);
    ...
    expect(userInfo.role).toBe('design_assistant');
    expect(userInfo.token).toBe('mock_token_design_assistant');
  });

  test('Craft designer has specialized permissions', async ({ page }) => {
    // 测试"工艺师傅"具有特殊权限
    await loginAs(page, 'craft_designer');
    ...
    expect(userInfo).toBe('craft_designer');
  });

  test('BOM manager has full access', async ({ page }) => {
    // 测试"BOM管理员"具有完全访问权限
    await loginAs(page, 'bom_manager');
    ...
    expect(userInfo).toBe('bom_manager');
  });

  test('Role-based UI elements visibility', async ({ page }) => {
    // 测试基于角色的UI元素可见性
    await loginAs(page, 'design_assistant');
    ...
    // 验证没有出现错误信息
    const errorElements = page.locator('.error, .el-message--error, [class*="error"]');
    const errorCount = await errorElements.count();
    
    if (errorCount > 0) {
      // 如果有错误元素，确保它们不可见或内容为空
